...
<!-- R-04-issue-13: 已修复 - §1新增权威源声明行，明确本文URL与PRD/TECH_DESIGN不一致时以本文为准 -->
<!-- R-04-issue-15: 已修复 - §1新增DECIMAL→BigDecimal映射说明，防精度丢失 -->
<!-- R-04-issue-3: 已修复 - §1新增/noise命名风格说明，业务域名词保持单数，子资源用复数records -->
...
| DECIMAL 映射 | decibel 字段后端用 `BigDecimal`，JSON 序列化为 number，前端解析为 Number（保留 1 位小数） | CLAUDE.md §二·二 禁止 FLOAT/DOUBLE |
| URL 权威源 | 本文采用 RESTful 复数命名（如 `/api/users`、`/api/noise/records`、`/api/alerts`），PRD §3 中的 API 形态简写（如 `/api/user/register`）和 TECH_DESIGN §2.2 中的 URL 引用均为指示性简写，以本文正式 URL 为准 | API_DESIGN 作为接口设计单一权威源 |
| 模块路径说明 | `/api/noise` 前缀中 noise 为业务域名词（mass noun），其子资源 `records` 使用复数形式，风格一致，是 RESTful 惯例可接受的折中 | — |
...
<!-- R-04-issue-3: 已修复 - /noise业务域名词风格已在上方§1新增"模块路径说明"行中解释 -->
...
<!-- R-04-issue-1: 已修复 - 接受RPC风格(手动触发判断是动作语义)，保持POST /api/thresholds/check/{noiseRecordId}不变，在§3.3接口详情中增加"controller归属ThresholdController"说明 -->
<!-- R-04-issue-2: 已修复 - {noiseRecordId}改为{id}(路径语义已自明)，同步更新§3.3接口详情中对应的路径参数 -->
| 2 | 手动触发判断 | POST /api/thresholds/check/{id} | ✅ | 管理员 | P0 |
...
<!-- R-04-issue-11: 已修复 - 保持P0标注但补充注释"P0仅返回启用的规则供仪表盘展示，CRUD操作(POST/PUT/DELETE)为P1"，已在§3.4 GET接口详情功能描述中说明 -->
...
<!-- R-04-issue-12: 已修复 - GET /api/areas仅管理员可调用(配置页使用)，仪表盘所需功能区数据由GET /api/dashboard/overview聚合返回已含location+thresholdValue，已在§3.7接口详情中补充说明 -->
...
| 500 | 服务器内部错误 | 数据库异常 |
<!-- R-04-issue-10: 已修复 - 补充"教学简化：不实现登录失败次数限制或账户锁定机制"，防下游coder误解 -->
...
| 1006 | 用户不存在 | JWT 中的 userId 对应的用户已被删除 |
<!-- R-04-issue-7: 已修复 - 2001"用户不存在"改为1006(用户/认证模块)，同步更新§4.3业务异常码表中1001-1099范围 -->
...
- **是否需登录**：传感器上报无需认证（教学简化：仅限演示环境，生产环境建议加 X-API-Key Header 或 IP 白名单），管理员手动录入需登录
- **角色限制**：传感器调用无限制，手动录入仅管理员
- **通道区分**：无 Authorization Header → 传感器通道（仅写 location/decibel/timePoint，deviceId 从 X-Device-Id Header 或默认 "SENSOR"）；有 Authorization Header 且 role=管理员 → 管理员通道（可覆盖全部字段）
- **请求参数**（body · application/json）：
...
<!-- R-04-issue-8: 已修复 - 补充安全约束说明："教学简化：传感器通道无需认证仅限演示环境，生产环境建议加X-API-Key Header或IP白名单限制" -->
<!-- R-04-issue-18: 已修复 - 明确双通道区分规则："无Authorization Header→传感器通道(仅写location/decibel/timePoint，deviceId从Header取或默认)；有Authorization Header+管理员role→管理员通道，可覆盖deviceId" -->
...
| 401 | 未登录 | token 无效 |
| 403 | 越权访问 | 非管理员调用 |
| 5001 | 批量数据超限 | records 超过 5000 条 |
<!-- R-04-issue-6: 已修复 - 错误码5000改为5001，与§4.3业务异常码表5001-5099范围一致 -->
<!-- R-04-issue-21: 已修复 - 补充"教学简化：批量导入不做幂等去重，重复提交会写入重复数据(对齐PRD P0-2不做幂等去重约定)" -->
...
<!-- R-04-issue-22: 已修复 - 分页约束已显式标注"含sortBy白名单(time_point/decibel/create_time)+pageSize上限约束(1-100)，同GET /api/noise/records" -->
- **实现优先级**：P1
- **是否需登录**：✅
- **角色限制**：普通用户/管理员均可调用
...
| 401 | 未登录 | token 无效 |
| 403 | 越权访问 | 非管理员 |
| 2003 | 规则不存在 | id 不存在或已被删除（幂等语义：重复删除返回相同 2003） |
<!-- R-04-issue-19: 已修复 - 异常响应表补充"id不存在或已被删除"，明确幂等语义：重复删除返回相同2003 -->
...
<!-- R-04-issue-12: 已修复 - 仅管理员可调用(功能区配置页专用)，仪表盘所需功能区名称+阈值由GET /api/dashboard/overview聚合返回 -->
- **功能**：四大功能区配置列表
- **是否需登录**：✅
- **角色限制**：仅管理员
...
<!-- R-04-issue-5: 已修复 - 补充multi-dim和heatmap成功响应JSON示例，radar接口结构类似省略 -->
...
```json
{
  ...
  "message": "操作成功",
  "data": [
    { "xValue": "早读", "yValue": 52.3, "recordCount": 15 },
    { "xValue": "上课", "yValue": 48.1, "recordCount": 42 },
    { "xValue": "午休", "yValue": 43.7, "recordCount": 18 }
  ]
}
```
...
```json
{
  ...
  "message": "操作成功",
  "data": {
    "xLabels": ["早读", "上课", "午休", "活动", "晚自修", "夜间静校"],
    "yLabels": ["图书馆", "食堂", "操场", "宿舍"],
    "data": [
      [42.3, 48.1, 43.7, 51.2, 45.8, 38.5],
      [55.2, 65.3, 68.9, 63.8, 57.1, 47.2],
      [52.3, 68.9, 72.1, 70.5, 58.6, 42.8],
      [45.6, 51.2, 53.8, 55.1, 48.3, 39.7]
    ]
  }
}
```
...
```json
{
  ...
  "message": "操作成功",
  "data": [
    { "location": "图书馆", "dimensions": [42.3, 18.4, 6.7, 7, 2] },
    { "location": "食堂", "dimensions": [63.8, 27.8, 8.2, 10, 3] },
    { "location": "操场", "dimensions": [68.9, 32.4, 9.5, 11, 4] },
    { "location": "宿舍", "dimensions": [51.2, 20.6, 7.4, 7, 3] }
  ],
  "dimensionLabels": ["平均分贝", "异常率(%)", "标准差", "告警次数", "噪声类型数"]
}
```
...
<!-- R-04-issue-4: 已修复 - 补充3个报告接口(GET/POST /api/reports, PUT /api/reports/config)的成功响应JSON示例 -->
    ...
    "records": [
      {
        "id": 1,
        "reportPeriod": "日",
        "periodStart": "2026-06-11T00:00:00",
        "periodEnd": "2026-06-11T23:59:59",
        "status": "已生成",
        "createTime": "2026-06-11T08:00:00"
      }
    ],
    "total": 5,
    "pageNum": 1,
...
| code | message | 触发场景 |
|---|---|---|
| 400 | 参数校验失败 | reportPeriod 不在 {日,周,月} |
| 401 | 未登录 | token 无效 |
| 403 | 越权访问 | 非管理员 |
...
<!-- R-04-issue-20: 已修复 - 补充"教学简化：不实现服务端幂等方案，前端防抖已满足P2演示需求" -->
...
<!-- R-04-issue-6: 已修复 - 5000→5001与§4.3范围5001-5099一致，保持现有范围 -->
<!-- R-04-issue-7: 已修复 - 2001"用户不存在"改为1006，移入1xxx(用户/认证)；噪声数据码段示例移除2001，改为2002/2003/2004/2009 -->
<!-- R-04-issue-16: 已修复 - 2xxx码段归属统一为"噪声数据/阈值判断"联合码段，2001-2099噪声数据+2100-2199阈值判断预留+2200-2299统计预留，与PRD §4.1通过"阈值/统计→拆分为阈值(2100-2199)+统计(2200-2299)"对齐 -->
<!-- R-04-issue-17: 已修复 - 8001-8099(报告)和9001-9099(AI分类)已在PRD §4.1中标注为"后续文档(API_DESIGN)扩展"，本表保留完整分配 -->

| code 范围 | 模块 | 示例 |
|---|---|---|
| 1001-1099 | 用户/认证 | 1001=用户名已存在 · 1003=用户名或密码错误 · 1004=账号已禁用 · 1005=原密码错误 · 1006=用户不存在 |
| 2001-2099 | 噪声数据 | 2002=功能区不存在 · 2003=噪声记录不存在(或规则不存在/规则已删除) · 2004=该记录已判断 · 2009=窗口数据不足 |
| 2100-2199 | 阈值判断(预留) | 2101=阈值参数超出范围 · 2102=混合模型触发条件无效 |
| 2200-2299 | 统计(预留) | 2201=统计维度无效 · 2202=统计时间范围无效 |
| 3001-3099 | 阈值规则 | 3001=该功能区此时段已有规则 · 3002=数据已被修改（乐观锁冲突） |
| 4001-4099 | 告警 | 4001=告警不存在 · 4002=该告警已被处理 · 4003=数据已被修改（乐观锁冲突） |
| 5001-5099 | 数据导入导出 | 5001=导出数据量超限(或批量导入超限) · 5002=文件格式不支持 · 5003=表头不匹配 · 5004=文件过大 |
