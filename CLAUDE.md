...
你是一个 SpringBoot 3 + Vue 3 + Python Flask 全栈开发助手。本项目是**校园噪音分贝预警员系统**,用户角色:**普通用户 + 管理员**。
...
**Phase 0 — 项目初始化** (2026-06-11)
- ⬜ PRD.md — 待生成(Phase 1)
- ⬜ TECH_DESIGN.md — 待生成(Phase 1)
- ⬜ DATABASE_DESIGN.md + sql/01-init.sql — 待生成(Phase 2)
- ⬜ API_DESIGN.md — 待生成(Phase 3)
- ⬜ 后端开发 — 待开始(Phase 4)
- ⬜ 前端开发 — 待开始(Phase 5)
- ⬜ 集成调试 — 待开始(Phase 6)
- ⬜ DEPLOY.md — 待生成(Phase 8)
...
对齐 `TECH_DESIGN.md §3 路由表` + `TECH_DESIGN.md §6 页面路由表`:
...
| LoginPage | `/login` | P0 |
| DashboardPage | `/` | P0 |
| NoiseMonitorPage | `/noise-monitor` | P0 |
| AlertHistoryPage | `/alert-history` | P0 |
| ThresholdConfigPage | `/threshold-config` | P1 |
| StatisticsPage | `/statistics` | P1 |
| AreaConfigPage | `/area-config` | P0 |
| SystemSettingsPage | `/settings` | P2 |
...
- **噪声数据采集与存储**(P0): 接收/模拟噪声传感器数据，存储历史记录，支持批量导入
- **业务规则动态阈值判断**(P0): 按"功能区+时段"二维规则字典计算动态阈值，超阈值标记异常
- **实时噪声监测仪表盘**(P0): 各功能区实时分贝值展示、当前阈值显示、异常状态指示灯
- **异常告警记录与推送**(P0): 异常发生时记录告警日志，支持列表分页查询
- **功能区配置管理**(P0): 管理四大功能区基础信息（名称、噪声敏感度、默认阈值范围）
- **统计自适应阈值计算**(P1): 滑动窗口计算动态阈值上下限，支持配置窗口大小和k值
- **混合阈值模型**(P1): 融合业务规则+统计自适应，最优方案准确率≥92%
- **历史数据可视化**(P1): ECharts时间序列图+各功能区统计图表+模型性能对比图
- **阈值规则配置界面**(P1): 可视化配置功能区时段阈值规则，对接ccswitch配置服务热更新
- **数据导入导出**(P2): 支持CSV/Excel导入噪声数据、导出报表
- **AI辅助噪声分类**(P2): 接入AI模型对噪声类型分类（交谈、施工、体育活动等）

### 五·二 关键路径速查(高频文件位置)

后端:
- 入口: `backend/src/main/java/com/example/noise/Application.java`
...
CCSwitch 配置服务:
- 入口: `ccswitch_service/app.py`(Flask 主应用)
- 配置读取: `ccswitch_service/ccswitch.py`(读取 ~/.claude/settings.json)
- 配置管理: `ccswitch_service/config.py`(ccswitch优先 + .env回退)
- 阈值规则: `ccswitch_service/threshold_rules.json`(动态阈值持久化配置)
