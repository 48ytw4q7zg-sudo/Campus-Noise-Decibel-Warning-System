...
CREATE TABLE IF NOT EXISTS user (
    ...
    role        VARCHAR(10)     NOT NULL DEFAULT '普通用户' COMMENT '角色：普通用户/管理员（应用层校验）',
...
CREATE TABLE IF NOT EXISTS alert_log (
    ...
    confirm_status   VARCHAR(10)     NOT NULL DEFAULT '未确认' COMMENT '确认状态：未确认→已确认→已处置，不允许回退',
...
CREATE TABLE IF NOT EXISTS report (
    ...
    status        VARCHAR(10)    NOT NULL DEFAULT '生成中' COMMENT '状态：生成中/已生成/生成失败',
